...
class GroupRepository(BaseRepository):
    def get_all(self) -> List[Dict]:
        ...
        # Ensure groups exist in data
        if "groups" not in data:
            data["groups"] = []
            self._write_data(data)
            
        # Add accounts count to each group
        for group in groups:
            group["accounts"] = self.get_accounts_by_group(group["id"])
    ...
    def get_by_id(self, group_id: int) -> Optional[Dict]:
        ...
        if group:
            group["accounts"] = self.get_accounts_by_group(group_id)
        return group

    def get_accounts_by_group(self, group_id: int) -> List[Dict]:
        data = self._read_data()
        return [a for a in data.get("accounts", []) if a.get("group_id") == group_id]
